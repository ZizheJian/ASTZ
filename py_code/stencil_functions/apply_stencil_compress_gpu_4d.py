...
def apply_stencil_compress_gpu_4d(args:args_c,stencil_manager:stencil_manager_c):
    args.cur_shape_list=[]
    ...
    tgt_data=torch.zeros([1,1,1,1,1,1],dtype=torch.float32,device=args.device)
    cur_data[0,0,0,0,0,0]=tgt_data[0,0,0,0,0,0]=args.data[0,0,0,0]
    mask=torch.zeros([1,2,1,1,1,1],dtype=torch.bool,device=args.device)
    mask[0,0,0,0,0,0]=True
    args.qb=torch.zeros(args.data_shape[0]*args.data_shape[1]*args.data_shape[2]*args.data_shape[3],dtype=torch.int32,device=args.device)
    args.qb_begin=args.qb_end=0
    pred_gap=(2**np.ceil(np.log2(args.data_shape))).astype(np.int32)
    args.pivot=torch.zeros(args.data_shape[0]*args.data_shape[1]*args.data_shape[2]*args.data_shape[3],dtype=torch.float32,device=args.device)
    args.pivot[0]=args.data[0,0,0,0]
    args.pivot_num=1
    for i in range(len(args.stencil_id_list)-1,-1,-1):
        ...
        cur_data,tgt_data,mask,pred_gap=expand_data_gpu_4d(cur_data,tgt_data,mask,pred_gap,args,cur_shape)
        ...
        for block_id,cur_block_pad,tgt_block,mask_block_pad in blockify_4d(cur_data,tgt_data,mask,args,padding=padding):
        ...
        block_num=[(cur_data.shape[2]+args.model_block_step[0]-1)//args.model_block_step[0],
                   ...
                   (cur_data.shape[5]+args.model_block_step[3]-1)//args.model_block_step[3]]
        ...
        blockify_records=[]
        for block_pos,cur_block_pad,tgt_block,mask_block_pad in blockify_4d(cur_data,tgt_data,mask,args,padding=padding):
            block_id=tuple(block_pos[i]//args.model_block_step[i] for i in range(4))
            ...
            mat_A[block_id],mat_B[block_id]=generate_matAB_gpu_4d(cur_block_ext,tgt_block,mask_block,mask_core,max_tgt_num,param_num,args)
        ...
        lstsq_result=torch.linalg.lstsq(mat_A_sampled,mat_B_sampled,driver="gels")
        mat_X=lstsq_result.solution
        mat_X_bin,mat_X=quantize_parameter_with_baseline(mat_X,mat_X_baseline.reshape(1,1,1,1,-1).expand_as(mat_X),args)
        ...
        for block_id,block_pos,cur_block,tgt_block,mask_block in blockify_records:
            ...
            cur_data[:,:,block_pos[0]:block_pos[0]+args.model_block_step[0],
                         ...
                         block_pos[2]:block_pos[2]+args.model_block_step[2],
                         block_pos[3]:block_pos[3]+args.model_block_step[3]]=cur_block
    ...
    if args.analysis:
        print(f"Huffman CR= {args.data_shape[0]*args.data_shape[1]*args.data_shape[2]*args.data_shape[3]*args.unit_size/len(hf_bs):.3f}")
        print(f"Zstd CR= {len(hf_bs)/len(args.zstd_bs):.3f}")
        print(f"\033[31mTotal CR= {args.data_shape[0]*args.data_shape[1]*args.data_shape[2]*args.data_shape[3]*args.unit_size/len(args.zstd_bs):.3f} \033[0m")
