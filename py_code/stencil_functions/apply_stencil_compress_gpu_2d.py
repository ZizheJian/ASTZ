...
from stencil_functions.expand_data import expand_data_gpu_2d
from stencil_functions.blockify import blockify_2d
from stencil_functions.generate_cur_block_ext import generate_cur_block_ext_gpu_2d
from stencil_functions.generate_matAB import generate_matAB_gpu_2d
from quantize import quantize_tensor,quantize_parameter_with_baseline
from read_write_dataset import restore_data_range

def apply_stencil_compress_gpu_2d(args:args_c,stencil_manager:stencil_manager_c):
    args.cur_shape_list=[]
    args.stencil_id_list=[]
    with open(args.stencil_path,"r") as f:
        for line in f:
            args.cur_shape_list.append([int(line.split()[0]),int(line.split()[1])])
            args.stencil_id_list.append(int(line.split()[2]))
    cur_data=torch.zeros([1,1,1,1],dtype=torch.float32,device=args.device)
    tgt_data=torch.zeros([1,1,1,1],dtype=torch.float32,device=args.device)
    cur_data[0,0,0,0]=tgt_data[0,0,0,0]=args.data[0,0]
    mask=torch.zeros([1,2,1,1],dtype=torch.bool,device=args.device)
    mask[0,0,0,0]=True
    args.qb=torch.zeros(args.data_shape[0]*args.data_shape[1],dtype=torch.int32,device=args.device)
    args.qb_begin=args.qb_end=0
    pred_gap=(2**np.ceil(np.log2(args.data_shape))).astype(np.int32)
    args.pivot=torch.zeros(args.data_shape[0]*args.data_shape[1],dtype=torch.float32,device=args.device)
    args.pivot[0]=args.data[0,0]
    args.pivot_num=1
    for i in range(len(args.stencil_id_list)-1,-1,-1):
        ...
        cur_data,tgt_data,mask,pred_gap=expand_data_gpu_2d(cur_data,tgt_data,mask,pred_gap,args,cur_shape)
        for i0,i1 in product(range(0,2),repeat=2):
            mask[:,1,i0::2,i1::2]=ref_pos[i0,i1]
        if mask.sum().item()<args.data_shape[0]*args.data_shape[1]/args.pivot_ratio:
        ...
        for block_id,cur_block_pad,tgt_block,mask_block_pad in blockify_2d(cur_data,tgt_data,mask,args,padding=padding):
        ...
        blockify_records=[]
        for block_pos,cur_block_pad,tgt_block,mask_block_pad in blockify_2d(cur_data,tgt_data,mask,args,padding=padding):
            block_id=tuple(block_pos[i]//args.model_block_step[i] for i in range(2))
            cur_block_ext=generate_cur_block_ext_gpu_2d(cur_block_pad,padding,args)
    ...
    if args.analysis:
        ...
        print(f"\033[31mTotal CR= {args.data_shape[0]*args.data_shape[1]*args.unit_size/len(args.zstd_bs):.3f} \033[0m")
