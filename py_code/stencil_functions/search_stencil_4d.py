...
from stencil_functions.check import num_of_reference_points_mismatch_check_4d,any_pred_tgt_out_of_boundary_check_4d,any_pred_tgt_processed_check_4d
from stencil_functions.blockify import blockify_4d
from stencil_functions.generate_cur_block_ext import generate_cur_block_ext_4d
from stencil_functions.generate_matAB import generate_matAB_4d
from quantize import quantize_parameter_with_baseline
from stencil_functions.shrink_data import shrink_data_4d

def search_stencil_4d(args:args_c,stencil_manager:stencil_manager_c):
    ...
    for i in range((np.sum(np.ceil(np.log2(args.data_shape)))).astype(int)):
        ...
        for stencil_id,stencil in stencil_manager.stencil_dict.items():
            ...
            if num_of_reference_points_mismatch_check_4d(ref_pos,mask):
                continue
            if any_pred_tgt_out_of_boundary_check_4d(ref_pos,tgt_data.shape):
                continue
            if any_pred_tgt_processed_check_4d(ref_pos,mask):
                continue
            for i0,i1,i2,i3 in product(range(0,2),repeat=4):
                mask[0,0,i0::2,i1::2,i2::2,i3::2]&=(~ref_pos[i0,i1,i2,i3])
                mask[0,1,i0::2,i1::2,i2::2,i3::2]=ref_pos[i0,i1,i2,i3]
            cur_data=torch.zeros_like(tgt_data)
            cur_data[mask[:,0:1]]=tgt_data[mask[:,0:1]]
            total_loss=0
            total_loss_num=0
            for block_id,cur_block_pad,tgt_block,mask_block_pad in blockify_4d(cur_data,tgt_data,mask,args,padding):
                cur_block_ext=generate_cur_block_ext_4d(cur_block_pad,padding,args)
                ...
                mat_A,mat_B=generate_matAB_4d(cur_block_ext,tgt_block,mask_block,mask_core,tgt_num,param_num,args)
                if args.method=="FIX":
            ...
            for i0,i1,i2,i3 in product(range(0,2),repeat=4):
                mask[0,0,i0::2,i1::2,i2::2,i3::2]|=ref_pos[i0,i1,i2,i3]
                mask[0,1,i0::2,i1::2,i2::2,i3::2]=False
        ########保存最佳topology########
        with open(args.stencil_path,"a") as f:
            f.write(f"{tgt_data.shape[2]} {tgt_data.shape[3]} {tgt_data.shape[4]} {tgt_data.shape[5]} {best_stencil_id}\n")
        tgt_data[best_mask[:,1:2]]=0
        mask[0,0]=best_mask[0,0]
        mask[0,1]=False
        tgt_data,mask=shrink_data_4d(tgt_data,mask,args)
