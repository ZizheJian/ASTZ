...
def apply_stencil_decompress_gpu_2d(args:args_c,stencil_manager:stencil_manager_c):
    ...
    for i in range(len(args.stencil_id_list)-1,-1,-1):
        ...
        cur_data,_,mask,pred_gap=expand_data_gpu_2d(cur_data,None,mask,pred_gap,args,cur_shape)
        ...
        for block_id,cur_block_pad,_,mask_block_pad in blockify_2d(cur_data,None,mask,args,padding=padding):
            ...
            mat_A,_=generate_matAB_gpu_2d(cur_block_ext,None,mask_block,mask_core,tgt_num,param_num,args)
            ...
            cur_data[:,:,block_id[0]:block_id[0]+args.model_block_step[0],
                         block_id[1]:block_id[1]+args.model_block_step[1]]=cur_block
        mask[:,0:1]+=mask[:,1:2]
    ...
    if args.analysis:
        ...
        print(f"\033[31mcr= {args.data_shape[0]*args.data_shape[1]*args.unit_size/len(args.zstd_bs):.3f} \033[0m")
