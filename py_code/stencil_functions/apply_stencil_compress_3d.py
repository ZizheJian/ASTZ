...
def apply_stencil_compress_3d(args:args_c,stencil_manager:stencil_manager_c):
    ...
    for i in range(len(args.stencil_id_list)-1,-1,-1):
        ...
        ref_pos=stencil_manager.stencil_dict[stencil_id].ref_pos
        mask_core=stencil_manager.stencil_dict[stencil_id].mask_core
        cur_data,tgt_data,mask,pred_gap=expand_data_3d(cur_data,tgt_data,mask,pred_gap,args,cur_shape)
        ...
        for block_id,cur_block_pad,tgt_block,mask_block_pad in blockify_3d(cur_data,tgt_data,mask,args,padding=padding):
            ...
            mat_A_sampled=torch.cat((mat_A[:-param_num][::args.sampling_gap],mat_A[-param_num:]),dim=0)
            mat_B_sampled=torch.cat((mat_B[:-param_num][::args.sampling_gap],mat_B[-param_num:]),dim=0)
            if args.method=="FIX":
            ...
            if args.method=="FHDE":
                lstsq_result=torch.linalg.lstsq(mat_A_sampled,mat_B_sampled,driver="gels")
                mat_X=lstsq_result.solution
                err=mat_A_sampled[:-param_num]@mat_X-mat_B_sampled[:-param_num]
                valid_equations=(err.abs()<=args.abs_eb*args.FHDE_threshold)
                if valid_equations.sum().item()>0:
                    mat_A_filtered=torch.cat((mat_A_sampled[:-param_num][valid_equations],mat_A_sampled[-param_num:]),dim=0)
                    mat_B_filtered=torch.cat((mat_B_sampled[:-param_num][valid_equations],mat_B_sampled[-param_num:]),dim=0)
