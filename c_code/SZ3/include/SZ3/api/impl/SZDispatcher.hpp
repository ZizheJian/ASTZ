...
#include "SZ3/api/impl/SZAlgoFHDE.hpp"
    ...
    if (conf.cmprAlgo != ALGO_LOSSLESS) {
        try {
            ...
            if (conf.cmprAlgo == ALGO_INTERP) {
                cmpSize = SZ_compress_FHDE<T, N>(conf, dataCopy.data(), cmpData, cmpCap);
            ...
            if (std::string(e.what()) == FHDE_ERROR_COMP_BUFFER_NOT_LARGE_ENOUGH) {
...
void SZ_decompress_dispatcher(Config &conf, const uchar *cmpData, size_t cmpSize, T *decData) {
    if (conf.cmprAlgo == ALGO_LOSSLESS) {
        ...
        SZ_decompress_FHDE<T, N>(conf, cmpData, cmpSize, decData);
    } else if (conf.cmprAlgo == ALGO_NOPRED) {
