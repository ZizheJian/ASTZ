...
enum ALGO { ALGO_HDE, ALGO_FHDE};
constexpr const char *ALGO_STR[] = {"ALGO_HDE", "ALGO_FHDE"};

constexpr const ALGO ALGO_OPTIONS[] = {ALGO_HDE, ALGO_FHDE};
   ...
   public:
    ...
    void loadcfg(const std::string &cfgpath) {
        ...
        if (!cmprAlgoStr.empty()) {
            if (cmprAlgoStr == ALGO_STR[ALGO_HDE]) {
                cmprAlgo = ALGO_HDE;
            } else if (cmprAlgoStr == ALGO_STR[ALGO_FHDE]) {
                cmprAlgo = ALGO_FHDE;
            } else {
