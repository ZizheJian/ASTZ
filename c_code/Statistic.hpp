#ifndef FHDE_STATISTIC_HPP
#define FHDE_STATISTIC_HPP
...
{
    if (conf.eb_mode!=EB_ABS)
        if (conf.eb_mode==EB_REL) 
        {
            conf.eb_mode=EB_ABS;
            conf.abs_eb=conf.rel_eb*((range>0)?range:data_range(data,conf.num));
        }
        else
        {
