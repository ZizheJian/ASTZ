...
template <class T>
void decompress(char *inPath, char *cmpPath, char *decPath, FHDE::Config conf, int binaryOutput, int printCmpResults) {
    size_t cmpSize;
    auto cmpData = FHDE::readfile<char>(cmpPath, cmpSize);
    T *decData = new T[conf.num];

    FHDE::Timer timer(true);
    FHDE_decompress<T>(conf, cmpData.get(), cmpSize, decData);
    double compress_time = timer.stop();
    ...
    if (binaryOutput == 1) {
        FHDE::writefile<T>(outputFilePath, decData, conf.num);
    } else {
        FHDE::writeTextFile<T>(outputFilePath, decData, conf.num);
    }
    if (true) {
        // compute the distortion / compression errors...
        size_t totalNbEle;
        auto ori_data = FHDE::readfile<T>(inPath, totalNbEle);
        assert(totalNbEle == conf.num);
        FHDE::verify<T>(ori_data.get(), decData, conf.num);
    }
    delete[] decData;
...
int main(int argc, char *argv[]) {
    ...
    if (r2 == 0) {
        ...
        conf = FHDE::Config(r2, r1);
    } else if (r4 == 0) {
        conf = FHDE::Config(r3, r2, r1);
    } else {
        conf = FHDE::Config(r4, r3, r2, r1);
    }
    ...
    if (errBoundMode != nullptr) {
        ...
        if (strcmp(errBoundMode, FHDE::EB_STR[FHDE::EB_ABS]) == 0) {
            conf.errorBoundMode = FHDE::EB_ABS;
            if (errBound != nullptr) {
                conf.absErrorBound = atof(errBound);
            }
        } else if (strcmp(errBoundMode, FHDE::EB_STR[FHDE::EB_REL]) == 0 || strcmp(errBoundMode, "VR_REL") == 0) {
            conf.errorBoundMode = FHDE::EB_REL;
            if (errBound != nullptr) {
                conf.relErrorBound = atof(errBound);
            }
        } else if (strcmp(errBoundMode, FHDE::EB_STR[FHDE::EB_PSNR]) == 0) {
            conf.errorBoundMode = FHDE::EB_PSNR;
            if (errBound != nullptr) {
                conf.psnrErrorBound = atof(errBound);
            }
        } else if (strcmp(errBoundMode, FHDE::EB_STR[FHDE::EB_L2NORM]) == 0) {
            conf.errorBoundMode = FHDE::EB_L2NORM;
            if (errBound != nullptr) {
                conf.l2normErrorBound = atof(errBound);
            }
        } else if (strcmp(errBoundMode, FHDE::EB_STR[FHDE::EB_ABS_AND_REL]) == 0) {
            conf.errorBoundMode = FHDE::EB_ABS_AND_REL;
        } else if (strcmp(errBoundMode, FHDE::EB_STR[FHDE::EB_ABS_OR_REL]) == 0) {
            conf.errorBoundMode = FHDE::EB_ABS_OR_REL;
        } else {
