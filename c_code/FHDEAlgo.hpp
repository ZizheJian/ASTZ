...
// #include "SZ3/compressor/FHDEGenericCompressor.hpp"
...
#include "def.hpp"
#include <cstddef>
#include "FHDEGenericCompressor.hpp"
#include "Lossless_zstd.hpp"
#include "Config.hpp"
...
// template <class T,uint N>
// void SZ_decompress_FHDE(const Config &conf,const uchar *cmpData,size_t cmpSize,T *decData) {
//     assert(conf.cmprAlgo == ALGO_INTERP);
//     auto cmpDataPos = cmpData;
//     auto sz = make_compressor_fhde_generic<T,N>(
//         make_decomposition_FHDE<T,N>(conf,LinearQuantizer<T>(conf.absErrorBound,conf.qb_cnt/2)),
...
//             T mean_guess;
//             lorenzo_config.qb_cnt = optimize_quant_invl_3d<T>(
...
//         if (conf.relErrorBound < 1.01e-6 && best_lorenzo_ratio > 5 && lorenzo_config.qb_cnt != 16384) {
//             auto quant_num = lorenzo_config.qb_cnt;
//             lorenzo_config.qb_cnt = 16384;
//             size_t sampleOutSize =
//                 SZ_compress_LorenzoReg<T,N>(lorenzo_config,sampling_data.data(),buffer,bufferCap);
//             //                delete[]cmprData;
//             ratio = sampling_num * 1.0 * sizeof(T)/sampleOutSize;
...
//                 best_lorenzo_ratio = ratio;
//             } else {
//                 lorenzo_config.qb_cnt = quant_num;
//             }
//         }
