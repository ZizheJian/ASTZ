...
FHDE::Config conf(100, 200, 300); // 300 is the fastest dimension
conf.cmprAlgo = FHDE::ALGO_INTERP_LORENZO;
conf.errorBoundMode = FHDE::EB_ABS; // refer to def.hpp for all supported error bound mode
conf.absErrorBound = 1E-3; // absolute error bound 1e-3
char *compressedData = SZ_compress(conf, data, outSize);

Interpolation example:
FHDE::Config conf(100, 200, 300); // 300 is the fastest dimension
conf.cmprAlgo = FHDE::ALGO_INTERP;
conf.errorBoundMode = FHDE::EB_REL; // refer to def.hpp for all supported error bound mode
conf.relErrorBound = 1E-3; // value-rang-based error bound 1e-3
char *compressedData = SZ_compress(conf, data, outSize);

Lorenzo/regression example :
FHDE::Config conf(100, 200, 300); // 300 is the fastest dimension
conf.cmprAlgo = FHDE::ALGO_LORENZO_REG;
...
size_t FHDE_compress(const FHDE::Config &config, const T *data, char *cmpData, size_t cmpCap) {
    ...
    size_t cmpDataLen = 0;
    if (conf.N == 1) {
        cmpDataLen = FHDE_compress_impl<T, 1>(conf, data, cmpDataPos, cmpDataCap);
    } else if (conf.N == 2) {
        cmpDataLen = FHDE_compress_impl<T, 2>(conf, data, cmpDataPos, cmpDataCap);
    } else if (conf.N == 3) {
        cmpDataLen = FHDE_compress_impl<T, 3>(conf, data, cmpDataPos, cmpDataCap);
    } else if (conf.N == 4) {
        cmpDataLen = FHDE_compress_impl<T, 4>(conf, data, cmpDataPos, cmpDataCap);
    } else {
 ...
 *
 * Similar with SZ_compress(FHDE::Config &conf, const T *data, char *cmpData, size_t cmpCap)
 * The only difference is this one doesn't need the pre-allocated buffer (thus remember to do 'delete []' yourself)
 */
template <class T>
char *FHDE_compress(const FHDE::Config &config, const T *data, size_t &cmpSize) {
    using namespace FHDE;

    size_t bufferLen = FHDE_compress_size_bound<T>(config);
    auto buffer = new char[bufferLen];
    cmpSize = FHDE_compress(config, data, buffer, bufferLen);
 ...
 */
template <class T>
void FHDE_decompress(FHDE::Config &config, const char *cmpData, size_t cmpSize, T *&decData) {
    using namespace FHDE;
    ...
    if (config.N == 1) {
        FHDE_decompress_impl<T, 1>(config, cmpDataPos, cmpDataSize, decData);
    } else if (config.N == 2) {
        FHDE_decompress_impl<T, 2>(config, cmpDataPos, cmpDataSize, decData);
    } else if (config.N == 3) {
        FHDE_decompress_impl<T, 3>(config, cmpDataPos, cmpDataSize, decData);
    } else if (config.N == 4) {
        FHDE_decompress_impl<T, 4>(config, cmpDataPos, cmpDataSize, decData);
    } else {
        fprintf(stderr, "Data dimension higher than 4 is not supported.\n");
        throw std::invalid_argument("Data dimension higher than 4 is not supported.");
    }
...
/**
 * API for decompression
 * Similar with SZ_decompress(FHDE::Config &config, char *cmpData, size_t cmpSize, T *&decData)
 ...
 */
template <class T>
T *FHDE_decompress(FHDE::Config &config, const char *cmpData, size_t cmpSize) {
    using namespace FHDE;
    T *decData = nullptr;
    FHDE_decompress<T>(config, cmpData, cmpSize, decData);
    return decData;
}
#endif
